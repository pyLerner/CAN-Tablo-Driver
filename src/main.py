...
- N байт: битовая маска пикселей области (непрерывный поток битов, row-major)
  * размер: ceil(width * height / 8); при width не кратной 8 следующая строка
    может начинаться внутри байта (не выравнивание строки на границу байта)
  * 0 -> черный пиксель
  * 1 -> нечерный пиксель (цвет из поля color_non_black)
  * неиспользуемые хвостовые биты только в конце всей маски = 0
"""
